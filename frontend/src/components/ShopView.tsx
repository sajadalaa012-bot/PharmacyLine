  ...
  const cartTabs = (
    <div className="flex items-center gap-1">
      {(["cart", "orders"] as const).map((tab) => (
        ...
        >
          {tab === "cart" ? t("common.cart") : t("common.myOrders")}
        </button>
      ))}
    </div>
  );
        ...
        <div className="mx-auto flex max-w-7xl items-center gap-4 px-5 py-4">
          ...
          <nav className="ms-auto flex shrink-0 items-center gap-1.5 sm:ms-0">
            ...
            <button
              ...
              className="relative ms-1 flex h-11 items-center gap-2 rounded-full bg-brand px-5 text-on-brand
                         transition hover:bg-brand-deep active:scale-[0.98]"
            >
              <ShoppingCart className="h-4 w-4" />
              <span className="label-caps hidden sm:inline">{t("common.cart")}</span>
              {cart.totalQty > 0 && (
      ...
      <section className={`shop-hero border-b border-line ${view === "store" ? "hidden sm:block" : ""}`}>
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-5 py-14 lg:grid-cols-[1.1fr_0.9fr] lg:py-20">
            ...
            <div className="mt-10 flex flex-wrap gap-x-7 gap-y-3">
              {TRUST.map(({ icon: Icon, key }) => (
                <div key={key} className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-ink-2">{t(key)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rise rounded-2xl border border-line bg-surface/70 p-7 backdrop-blur-sm lg:p-9">
            <span className="label-caps text-ink-3">{t("shop.inCollection")}</span>
            <div className="mt-4 flex items-end gap-6">
              <div>
                <p className="font-display text-5xl font-semibold leading-none tracking-tight text-ink tabular-nums">
                  {allProducts.length}
                </p>
                <p className="mt-1.5 text-xs text-ink-3">{t("shop.productsInStock")}</p>
              </div>
              ...
              <div>
                ...
                <p className="mt-1.5 text-xs text-ink-3">{t("shop.categories")}</p>
              </div>
            </div>
        ...
        <div className="mb-6 space-y-4">
          <div className="flex items-baseline justify-between gap-4">
            ...
            <span className="label-caps text-ink-3">
              ...
              {visibleProducts.length === 1 ? t("common.item") : t("common.items")}
            </span>
          </div>
        ...
        >
          ...
          >
            <span className="relative">
              ...
              {cart.totalQty > 0 && (
                <span className="absolute -end-2.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-copper px-1 text-[9px] font-bold text-white">
                  {cart.totalQty}
                </span>
              )}
            </span>
            <span className="text-[10px] font-semibold">{t("common.cart")}</span>
          </button>

          {/* Home — center, raised */}
          <button
            onClick={goHome}
            aria-label={t("shop.home")}
